...
STATUS freeConnectionListener(PConnectionListener* ppConnectionListener)
{
    ...
    if (pConnectionListener->connectionList != NULL) {
        CHK_LOG_ERR_NV(doubleListGetHeadNode(pConnectionListener->connectionList, &pCurNode));
        while(pCurNode != NULL) {
            pSocketConnection = (PSocketConnection) pCurNode->data;
            pCurNode = pCurNode->pNext;
            CHK_STATUS(freeSocketConnection(&pSocketConnection));
        }
        CHK_LOG_ERR_NV(doubleListClear(pConnectionListener->connectionList, FALSE));
